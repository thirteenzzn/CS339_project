...
line = line[:-1]
singleWifiList=line.split()
avgWifi[singleWifiList[0]]=singleWifiList[1]
while line:  # 直到读取完文件
    line = f.readline()  # 读取一行文件，包括换行符
    line = line[:-1]  # 去掉换行符
    if len(line)!=0: #如果这一行不是空行
    ...
    if i!=24:
        ...
        for Key in avgWifi.keys():
            if historyDate[i].__contains__(Key): #如果参考点i处有AP=Key的wifi信号
                Distance=Distance+(float(avgWifi[Key])-float(historyDate[i][Key]))**2
            else:
                Distance=Distance+float(avgWifi[Key])**2
        distancesDict[i]=Distance
